...
=== Export MPS SLanguage to JSON
Technical name: `ExportSLanguageToJson` / `M2ToJson` / `Language2JsonConverter`

Converts a list of MPS Languages to LionCore JSON.
We use MPS' _Runtime Language_ representation.

[[mps-language-json-export]]
=== Export MPS Language Structure to JSON
Technical name: `ExportMpsLanguageStructureToJson` / `M2ToJsonMps` / `MpsLanguage2JsonConverter`

Converts a list of MPS Languages to LionCore JSON.
We use MPS' _structure aspect_ representation.
...
Technical name: `ExportInstanceToJson` / `M1ToJson` / `AMps2LionWebConverter`
...
We can opt to include _unconverted parent ids_.
Assume node `A` has node `B` as child.
According to the scoping rules, only `B` will be exported.
If enabled (`unconverted parent ids = true`), Child `B` will mention `A` as its parent.

We can opt to include _computed properties_.
If enabled (`computed properties = true`), and the property has a custom getter, the computed property value will be exported.


[[instance-json-import]]
=== Import Instance from JSON
Technical name: `ImportInstanceFromJson` / `JsonToM1` / `MergingLionWeb2MpsConverter`
